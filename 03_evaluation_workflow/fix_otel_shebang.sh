...
# Run this AFTER `agentcore launch` and BEFORE the final deploy
...
if [ ! -f "$DEPS_ZIP" ]; then
    ...
    echo "Run 'agentcore launch' first to create the dependencies cache"
    exit 1
fi 
...
echo "Now run 'agentcore launch' to deploy with the fixed dependencies"