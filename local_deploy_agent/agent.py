...
async def stream_response(prompt: str) -> AsyncGenerator[str, None]:
    """
    Stream agent response token by token with GuardRails intervention handling.
    
    GuardRails Intervention Handling:
    When GuardRails blocks content (either user input or model output), Bedrock raises
    an exception. This function catches these exceptions and returns a user-friendly
    ...
    In both cases, we log the intervention for monitoring and return a helpful message
    to the user without exposing internal error details.
    ...
    try:
        ...
        if any(keyword in error_msg for keyword in ["guardrail", "intervention", "blocked"]):
            ...
            # For non-GuardRails errors, log and re-raise
...
async def invoke_agent(prompt: str) -> str:
    """
    Invoke agent and return full response (non-streaming) with GuardRails handling.
    
    This function provides the same GuardRails intervention handling as stream_response,
    but for non-streaming invocations. When GuardRails blocks content, it returns a
    user-friendly error message instead of raising an exception.
    
    Args:
        prompt: User's input prompt
        
    Returns:
        str: Agent's response or error message if intervention occurs
    """
    input_data = {"messages": [{"role": "user", "content": prompt}]}
