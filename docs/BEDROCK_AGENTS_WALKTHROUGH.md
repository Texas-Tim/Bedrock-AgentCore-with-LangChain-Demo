...
- **Bedrock Model Access** enabled for Claude Sonnet 4
  - Go to [AWS Console > Bedrock > Model Access](https://console.aws.amazon.com/bedrock)
...
- **Project dependencies** installed:
  ```bash
  cd agentcore_demo/local  # or deployed/
  python -m venv .venv
...
Set environment variables with your GuardRail ID and version:
...
```bash
echo 'export BEDROCK_GUARDRAIL_ID="gr-abc123xyz"' >> ~/.bashrc
echo 'export BEDROCK_GUARDRAIL_VERSION="1"' >> ~/.bashrc
source ~/.bashrc
```
...
**Scenario**: Block toxic or harmful content in user inputs and agent responses.
...
Here's how GuardRails are integrated in the agent code:
...
Use these test prompts to validate your GuardRails configuration:

**Test 1: Content Filter**
```
Prompt: [Try content that should be blocked based on your filters]
Expected: Intervention message
```

**Test 2: PII Detection**
```
Prompt: My SSN is 123-45-6789 and my email is test@example.com
Expected: PII redacted or blocked
```

**Test 3: Denied Topic**
```
Prompt: Should I invest in cryptocurrency?
Expected: Intervention message (if financial-advice topic is denied)
```

**Test 4: Normal Conversation**
```
Prompt: What is the weather like today?
Expected: Normal response (no intervention)
```
...
- Aim for 500-2000 words per document for optimal chunking

**Example Document Structure**:
```markdown
# Product Documentation: Widget X

## Overview
Widget X is a revolutionary product that...
...
3. **Configure Data Source**
   ...
   **Option A: S3 Data Source** (Recommended)
   ...
   **Option B: Web Crawler Data Source**
   - **Data source name**: `web-docs`
   - **Source URLs**: Enter website URLs to crawl
   - **Crawl scope**: Choose "Host only" or "Subdomains"
   - **Crawl depth**: Maximum depth to crawl (1-10)
...
4. **Configure Embeddings Model**
   ...
   **Recommended Models**:
   ...
   - **Cohere Embed English v3**
     - Dimensions: 1024
     - Max tokens: 512
     - Best for: English documents, faster retrieval
...
5. **Configure Vector Store**
   ...
   **Option B: Amazon Aurora PostgreSQL**
   - Requires existing Aurora cluster with pgvector extension
   - **Advantages**: Use existing database, lower cost for large datasets
   
   **Option C: Pinecone**
   - Requires Pinecone account and API key
   - **Advantages**: Specialized vector database, advanced features
...
Set environment variable with your Knowledge Base ID:
...
```bash
echo 'export BEDROCK_KNOWLEDGE_BASE_ID="KB123ABC"' >> ~/.bashrc
source ~/.bashrc
```
